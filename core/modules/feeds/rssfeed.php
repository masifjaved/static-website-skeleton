<?php
  ...
  $TestFeed->setTitle('New shell - RSS News Feed');
  $TestFeed->setLink('http://' . $_SERVER['HTTP_HOST']);
  $TestFeed->setDescription('All the latest news from New Shell');
  ...
  while ($a = $db->get_row()) {	
	...
	$newItem->addElement('All the latest news from New Shell',$txt);
